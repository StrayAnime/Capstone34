...
function setup() {
  ...
  tower = Bodies.rectangle(160, 420, 260, 310, { isStatic: true });
  World.add(world, tower);

  cannon = new Cannon(190, 190, 130, 100, angle);
...
function showZombies() {
  if (zombies.length > 0) {
    if (
      zombies[zombies.length - 1] === undefined ||
    ...
    for (var i = 0; i < zombies.length; i++) {
      if (zombies[i]) {
        ...
        var collision=Matter.SAT.collides(zombies[i].body,tower)
    ...
    var zombie = new Zombie(width, height - 60, 170, 170, -160);
    zombies.push(zombie);
  }
}
